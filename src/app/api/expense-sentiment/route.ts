...
import { getOwnExpenseAmountsForDate, upsertSentiment } from "@/lib/expenseSentiment";
import { todayStrJST } from "@/lib/date";

const bodySchema = z.object({ sentiment: z.enum(["good", "bad"]) });

/** 今日の合計はクライアントから受け取らず、常にサーバー側で計算し直してから記録する。 */
export async function POST(req: Request) {
  try {
    const session = await requireOwnerSession();
    const { sentiment } = bodySchema.parse(await req.json());
    const date = todayStrJST();
    const amounts = await getOwnExpenseAmountsForDate(session.profile_id, date);
