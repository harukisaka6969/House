...
export async function POST(req: Request) {
  try {
    ...
    const { destination, date, start_time, end_time } = bodySchema.parse(await req.json());
    if (!isValidDateStr(date)) throw new ApiError(400, "invalid date");

    const result = await researchParkingOptions(destination, date, start_time, end_time);
