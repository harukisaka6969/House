...
${COMMON_RULE}${avoidRepeatClause(recentFull, olderSummaries)}`,
  ...
  },
  {
    ...
    onlyForSlug: "haruki",
    buildPrompt: (recentFull, olderSummaries) =>
      `あなたはメンズファッションスタイリストです。ユーザーは25歳男性で、体型は少し筋肉質でやや恰幅の良い（がっしりして少しふくよかな）体型です。この体型に似合う、今日から試せる服の色とシルエットの組み合わせを1つだけ提案してください。長い説明は不要です。次の形式で、絵文字を使ってできるだけ短く・見た目でパッと分かるようにまとめてください（各行は絵文字と単語だけ。理由の説明は最後の1行のみ）。
