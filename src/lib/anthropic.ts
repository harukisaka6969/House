 ...
 * 間食やデザート・飲み会など嗜好性の高いもの）はa3として扱う。a5は、毎月繰り返さない・覚悟を持って
 * 買うような単発の高額出費（パソコン購入など）専用で、日用品・趣味の範囲に収まる通常の買い物では
 * 選ばないこと（判断に迷ったらa5にはせず、a1かa3を優先する）。 */
function accountRuleText(acctList: string): string {
  return `口座候補: ${acctList}。内容から最も適切な口座を選ぶこと（日常の生活必需品はa1、ローン返済はa2、趣味・娯楽・交際・レジャーはa3、投資関連はa4、毎月繰り返さない・覚悟を持って買うような単発の高額出費（パソコン購入など、通常の月次予算には含まれない特別な買い物）はa5。外食は、普段の値段の必須の食事（朝食・昼食・夕食）ならa1、それ以外（高額な食事、カフェ・喫茶店、間食・デザート、飲み会など嗜好性の高いもの）はa3として扱うこと。a5は明らかに特別な高額出費のときだけ選び、判断がつかなければa1）。`;
}
...
export async function classifyLineText(text: string): Promise<LineTextIntent> {
  const res = await anthropic().messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `次のメッセージは「食事の内容」「支出（単に買い物などお金を使った内容）」「収入（お金が入った内容）」「家電の操作やシーンの実行（照明・エアコン・鍵など）」「節約アクション（工夫して支出を抑えた・安く買った・ポイントを使ったなど、節約につながる行動の報告）」「筋トレ・運動の記録（種目名と重量・回数、または時間・距離などの報告）」「それ以外」のどれに一番近いですか。単なる買い物の報告はexpense、割引で買った・自炊で節約した・ポイントで支払ったなど「工夫して安くした」という要素があればsavingsです。「ヨーグルト300グラム」「バナナ1本」のように、金額を伴わず食材名と分量・個数だけが書かれている（「食べた」等の動詞が無い）場合もmealです。meal / expense / income / smarthome / savings / gym / unknown のいずれか1単語のみを返してください。\nメッセージ: ${text}`,
      },
    ],
  });
