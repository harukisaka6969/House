...
function extractMetricSeries(records: PersonalRecordRow[], label: string): { date: string; value: number }[] {
 ...
 * 2. 赤字幅は基本15%。筋トレの総挙上量が伸びていれば控えめに、筋肉量が実際に減っていれば
 *    さらに絞る（10%を上限）ことで、筋肉を守りながら体脂肪を減らす方向に倒す。
...
export async function recomputePfcTargetFromBodyRecords(ownerId: string): Promise<PfcRecommendation | null> {
  const { data, error } = await db().from("personal_records").select("*").eq("owner", ownerId).order("date", { ascending: true });
  ...
  if (muscleChangeKg !== null && muscleChangeKg < -0.2) deficitPct = Math.min(deficitPct, 0.1);
  ...
  if (muscleChangeKg !== null && muscleChangeKg < -0.2) {
